use std::fs::read_to_string;
    ...
    fn main() -> io::Result<()> {
      let input = read_to_string("/dev/problems/fruit-into baskets/tests/inputs/##INPUT_FILE_INDEX##.txt")?;
      let mut lines = input.lines();
      let size_fruits: usize = lines.next().and_then(|line| line.parse().ok()).unwrap_or(0);
    let fruits: Vec<i32> = parse_input(&mut lines, size_fruits);
      let result = Solution::totalFruit(fruits);
      println!("{}", result);
      Ok(())
    }
