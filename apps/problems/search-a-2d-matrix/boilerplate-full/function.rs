use std::fs::read_to_string;
    ...
    fn main() -> io::Result<()> {
      let input = read_to_string("/dev/problems/search-a 2d matrix/tests/inputs/##INPUT_FILE_INDEX##.txt")?;
      let mut lines = input.lines();
      let size_matrix: usize = lines.next().and_then(|line| line.parse().ok()).unwrap_or(0);
    let matrix: Vec<Vec<i32>> = parse_input(&mut lines, size_matrix);
