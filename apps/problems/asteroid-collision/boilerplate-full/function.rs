use std::fs::read_to_string;
    ...
    fn main() -> io::Result<()> {
      let input = read_to_string("/dev/problems/asteroid-collision/tests/inputs/##INPUT_FILE_INDEX##.txt")?;
      let mut lines = input.lines();
      let size_asteroids: usize = lines.next().and_then(|line| line.parse().ok()).unwrap_or(0);
    let asteroids: Vec<i32> = parse_input(&mut lines, size_asteroids);
      let result = Solution::collision(asteroids);
