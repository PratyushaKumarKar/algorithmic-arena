use std::fs::read_to_string;
    ...
    fn main() -> io::Result<()> {
      let input = read_to_string("/dev/problems/remove-linked list elements/tests/inputs/##INPUT_FILE_INDEX##.txt")?;
      let mut lines = input.lines();
      
      let n_head: usize = lines.next().and_then(|line| line.parse().ok()).unwrap_or(0);
      let head = build_list_node(&mut lines, n_head);
  let val: i32 = lines.next().unwrap().parse().unwrap();
      let result = Solution::removeElements(head, val);
      println!("{}", result);
      Ok(())
    }
